...
class CRest
{
    ...
    protected static function callCurl($arParams)
    {
        ...
        if ($arSettings !== false) {
            ...
            try {
                ...
                if (defined("C_REST_IGNORE_SSL") && config('crest.C_REST_IGNORE_SSL') === true) {
    ...
    {
        if (defined("C_REST_WEB_HOOK_URL") && !empty(config('crest.C_REST_WEB_HOOK_URL'))) {
            $arData = [
                'client_endpoint' => config('crest.C_REST_WEB_HOOK_URL'),
                'is_web_hook' => 'Y'
            ];
            $isCurrData = true;
        } else {
    ...
    protected static function getSettingData()
    {
        ...
        if (file_exists(__DIR__ . '/settings.json')) {
            ...
            if (defined("C_REST_CLIENT_ID") && !empty(config('crest.C_REST_CLIENT_ID'))) {
                $return['C_REST_CLIENT_ID'] = config('crest.C_REST_CLIENT_ID');
            }
            if (defined("C_REST_CLIENT_SECRET") && !empty(config('crest.C_REST_CLIENT_SECRET'))) {
                $return['C_REST_CLIENT_SECRET'] = config('crest.C_REST_CLIENT_SECRET');
            }
        }
        return $return;
    }
    ...
    protected static function changeEncoding($data, $encoding = true)
    {
        if (is_array($data)) {
            ...
            if ($encoding) {
                $result = iconv(config('crest.C_REST_CURRENT_ENCODING'), "UTF-8//TRANSLIT", $data);
            } else {
                $result = iconv("UTF-8", config('crest.C_REST_CURRENT_ENCODING'), $data);
            }
        }
    ...
    {
        $return = false;
        if (!defined("C_REST_BLOCK_LOG") || config('crest.C_REST_BLOCK_LOG') !== true) {
            if (defined("C_REST_LOGS_DIR")) {
                $path = config('crest.C_REST_LOGS_DIR');
            } else {
                $path = __DIR__ . '/logs/';
            }
            $path .= date("Y-m-d/H") . '/';
            ...
            if (!defined("C_REST_LOG_TYPE_DUMP") || config('crest.C_REST_LOG_TYPE_DUMP') !== true) {
